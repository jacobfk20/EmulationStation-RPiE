...
Res2h::ResourceEntry Res2h::loadFileFromDisk(const std::string & filePath)
{
    ...
    if (inStream.is_open() && inStream.good()) {
        ...
        if (fileSize > 0) {
            //allocate data
            std::shared_ptr<unsigned char> fileData = std::shared_ptr<unsigned char>(new unsigned char[fileSize], [](unsigned char *p) {
                delete[] p;
            });
            //try reading data
            try {
                ...
                /*reading didn't work properly. salvage what we can.*/
            }
...
Res2h::ResourceEntry Res2h::loadFileFromArchive(const Res2h::ResourceEntry & entry)
{
    ...
    if (inStream.is_open() && inStream.good()) {
        ...
        //allocate data
        temp.data = std::shared_ptr<unsigned char>(new unsigned char[temp.dataSize], [](unsigned char *p) {
            delete[] p;
        });
        //try reading data
        try {
